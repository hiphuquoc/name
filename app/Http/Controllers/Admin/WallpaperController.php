...
class WallpaperController extends Controller {
    ...
    public function changeWallpaperWithSource(Request $request){
        ...
        //     DB::beginTransaction();
            ...
            if (!empty($request->file('wallpapers')) && !empty($request->file('sources')) && count($request->file('wallpapers')) == count($request->file('sources'))){
                /* xóa ảnh cũ */
                self::delete($infoWallpaper);
                /* upload lại === */
                $i              = 0;
                foreach ($wallpapers as $wallpaper) {
                    /* Lấy thông tin ảnh => trường hợp change thì chỉ xử lý một nhưng dữ liệu nằm trong mảng */
                    $imageInfo  = getimagesize($wallpaper);
                    $width      = $imageInfo[0];
                    $height     = $imageInfo[1];
                    $fileSize   = filesize($wallpaper);
                ...
                /* ảnh mini */
                $tmp            = pathinfo($infoWallpaper->file_url_hosting);
