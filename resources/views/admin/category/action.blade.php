<div class="actionBox">
    @if($language=='vi')
        ...
        <div class="actionBox_item maxLine_1" onclick="createMultiJobTranslateContent('{{ $item->seo->slug ?? '' }}');">
            <i class="fa-solid fa-language"></i>Dịch content tùy chọn (chạy ngầm)
        </div>
    @else   
        ...
        @endphp
        @if(!empty($itemSeo->id)&&!empty($itemSeo->jobAutoTranslate))
            <div class="actionBox_item maxLine_1" onClick="createJobTranslateContent({{ $item->seo->id }}, '{{ $language }}')">
                <i class="fa-solid fa-language"></i>Dịch content trang này (chạy ngầm)
            </div>
        @endif
